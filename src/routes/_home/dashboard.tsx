    ...
    <div className="flex flex-col gap-2">
      {streamMessages.map((message) => (
        <MessageItem
          key={message.id}
          message={
            message.Clipboard || message.GenericText || message.ConnectedDevices
          }
          type={message.type}
        />
      ))}
...
function MessageItem({
  ...
  message: ClipboardMessage | GenericTextMessage | ConnectedDevices | undefined;
  type: MessageType;
}) {
  function getMessage(): string | undefined {
    ...
    if (type === MessageType.CLIPBOARD) {
      ...
      return (message as ConnectedDevices).devices
